...
const TrainingProgramOwnershipCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            OrganizationName: _entity?.OrganizationName?._id,
OrganizationType: _entity?.OrganizationType,
OwnershipType: _entity?.OwnershipType,
        };

        setLoading(true);
        try {
            
        await client.service("trainingProgramOwnership").patch(_entity._id, _data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info trainingProgramOwnership updated successfully" });
    ...
    return (
        <Dialog header="Edit TrainingProgramOwnership" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="trainingProgramOwnership-edit-dialog-component">
