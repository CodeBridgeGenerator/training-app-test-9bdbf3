...
import trainingProgramOwnershipFakerFactory from "./trainingProgramOwnershipFakerFactory";
...
const TrainingProgramOwnershipFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [OrganizationNameIds, setOrganizationNameIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("businessInformation")
                        .find({ query: { $select: ["_id","OrganizationName"] } })
                        .then((data) => {
                        setOrganizationNameIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = trainingProgramOwnershipFakerFactory(props.user,fakerCount,OrganizationNameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("trainingProgramOwnership").create(elem));
...
export default connect(mapState, mapDispatch)(TrainingProgramOwnershipFakerDialogComponent);
