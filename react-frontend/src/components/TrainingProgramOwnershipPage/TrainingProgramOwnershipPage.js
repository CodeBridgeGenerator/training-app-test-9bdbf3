...
const TrainingProgramOwnershipPage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , OrganizationName : urlParams.singleBusinessInformationId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            ...
            console.log({ error });
            props.alert({ title: "TrainingProgramOwnership", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "TrainingProgramOwnership",
                type: "error",
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">TrainingProgramOwnership </h3>
                </div>
